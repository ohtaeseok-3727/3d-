...
void CMesh::Render(HDC hDCFrameBuffer)
{
	for (int j = 0; j < m_nPolygons; j++)
	{
		...
		// POINT 배열을 동적으로 할당 (또는 최대 정점 수를 안다면 고정 크기 배열 사용 가능)
		POINT* pPoints = new POINT[nVertices];
		bool bVisible = true; // 다각형의 모든 정점이 보이는지(가시성) 판단할 플래그

		for (int i = 0; i < nVertices; i++)
		{
			// 1. 3D 좌표를 2D 화면 좌표로 투영 변환
			XMFLOAT3 f3CurrentProject = CGraphicsPipeline::Project(pVertices[i].m_xmf3Position);

			// 2. 깊이(z) 값이 카메라 앞쪽(0.0 ~ 1.0)에 있는지 확인하여 하나라도 화면 뒤에 있으면 버림
			if (f3CurrentProject.z < 0.0f || f3CurrentProject.z > 1.0f)
			{
				bVisible = false;
				break;
			}

			// 3. 투영된 좌표를 뷰포트 기반 화면 픽셀 좌표로 변환
			XMFLOAT3 f3Screen = CGraphicsPipeline::ScreenTransform(f3CurrentProject);
			pPoints[i].x = (LONG)f3Screen.x;
			pPoints[i].y = (LONG)f3Screen.y;
		}

		// 4. 모든 정점이 카메라 앞에 있다면 GDI Polygon 함수를 이용해 채워진 다각형을 그림
		if (bVisible)
		{
